...
import com.chat.model.IMLoginInfo;
...
public class LoginProcessor extends AbstractMessageProcessor<IMLoginInfo> {
    ...
    public synchronized void process(ChannelHandlerContext ctx, IMLoginInfo loginInfo) {
        if (!JwtUtil.checkSign(loginInfo.getAuthorization(), authorizationSecret)) {
            ctx.channel().close();
            log.warn("用户token校验不通过，强制下线,token:{}", loginInfo.getAuthorization());
        }
        String strInfo = JwtUtil.getInfo(loginInfo.getAuthorization());
