...
public class PrivateMessageProcessor extends AbstractMessageProcessor<IMRecvInfo> {
    ...
    public void process(IMRecvInfo recvInfo) {
        ...
        IMUserInfo receiver = (IMUserInfo) recvInfo.getReceivers().get(0);
        log.info("接收到消息，发送者:{},接收者:{}，内容:{}", sender.getUserId(), receiver.getUserId(), recvInfo.getData());
        try {
    ...
    private void sendResult(IMRecvInfo recvInfo, IMSendCode sendCode) {
        if (recvInfo.getSendResult()) {
            ...
            result.setReceiver((IMUserInfo) recvInfo.getReceivers().get(0));
