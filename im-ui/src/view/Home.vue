		...
		methods: {
			init() {
				this.$store.dispatch("load").then(() => {
					...
					// ws初始化
					this.$wsApi.connect(process.env.VUE_APP_WS_URL, sessionStorage.getItem("authorization"));
					...
					this.$wsApi.onClose((e) => {
						...
						if (e.code != 3000) {   
							...
							this.$wsApi.reconnect(process.env.VUE_APP_WS_URL, sessionStorage.getItem("authorization"));
						}
					});
				}).catch((e) => {
					console.log("初始化失败", e);
