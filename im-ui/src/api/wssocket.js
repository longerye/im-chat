...
let connect = (wsurl,authorization) => {
	try {
		if (isConnect) {
			return;
		}
		...
		websock.onerror = function() {
			...
			reconnect(wsurl,authorization);
		}
	} catch (e) {
		console.log("尝试创建连接失败");
		reconnect(wsurl,authorization); //如果无法连接上webSocket 那么重新连接！可能会因为服务器重新部署，或者短暂断网等导致无法创建连接
	}
};

//定义重连函数
let reconnect = (wsurl,authorization) => {
	console.log("尝试重新连接");
	if (isConnect){
		//如果已经连上就不在重连了
		return; 
